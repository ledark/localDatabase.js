...
return"";},setCookie:function(cvalue,exdays){var cname="localDatabase";var d=new Date();d.setTime(d.getTime()+(exdays*24*60*60*1000));var expires="expires="+d.toUTCString();document.cookie=cname+"="+cvalue+"; "+expires;},isCookied:function(){if(this.getCookie("localDatabase")!=""){return true;}else{return false;}},setVar:function(nome,valor){var objRaw=this.get();var dados=JSON.parse(objRaw);eval("dados[0]."+nome+" = valor;");localDatabase.set(JSON.stringify(dados));},getVar:function(nome){var objRaw=this.get();var dados=JSON.parse(objRaw);eval("var item = dados[0]."+nome+";");return item;},delVar:function(nome){var objRaw=this.get();var dados=JSON.parse(objRaw);eval("delete dados[0]."+nome+";");localDatabase.set(JSON.stringify(dados));},clear:function(){this.del();this.delVar('campo1');this.delVar('campo2');}};$(function(){$('[localDatabase]').on('change',function(){var nome=$(this).attr('localDatabase');if(nome==''||nome==undefined)nome=$(this).attr('name');var valor=$(this).val();localDatabase.setVar(nome,valor);});$('[localDatabase]').each(function(){var nome=$(this).attr('localDatabase');if(nome==''||nome==undefined)nome=$(this).attr('name');var valor=localDatabase.getVar(nome);$(this).val(valor);});});
